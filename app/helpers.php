...
// this keeps latin characters and Persian characters as well
        $string = preg_replace("/[^a-z0-9_\s\-۰۱۲۳۴۵۶۷۸۹ءاآؤئبپتثجچحخدذرزژسشصضطظعغفقکكگلمنوهی]/u", '', $string);
...
if (! function_exists('persian_date_picker_to_timestamp')) {
    function persian_date_picker_to_timestamp($date_picker)
    {

        $full_date_array = explode(' - ' , $date_picker);
        $date_array = explode("/",$full_date_array[0]);
        $gregorian_date= jalali_to_gregorian($date_array[0] , $date_array[1] , $date_array[2]);
        $date_time_stamp = mktime(0,0,0,$gregorian_date[1],$gregorian_date[2],$gregorian_date[0]);

        $time_array = explode(":",$full_date_array[1]);

        $time_stamp = $date_time_stamp + fa_to_en($time_array[0])*60*60 + fa_to_en($time_array[1])*60;
...
if (! function_exists('image_absolute_path')) {
    function image_absolute_path($relative_path)
    {
        return $absolute_path = config("app.class_laravel_url").'/warehouse/images/' .$relative_path ;
    }
}
